...
public class CeptreWriter
{
    ...
    private void WriteCeptreFile(string fileName)
    {
        ...
        string path = string.Format(@"{0}\{1}\{2}\{3}.cep", resourcesFolder, ceptreDirectory, outputDirectory, fileName);

        // Safety check, a trace is required to write a program
        if (string.IsNullOrEmpty(_trace))
        {
            Console.WriteLine("Can't write file, TRACE is not specified");
            return;
        }
        ...
        Console.WriteLine($"Done writing file {path}");
    }
    ...
    private void CreateTrace(string stageName, string contextName)
    {
        if (!string.IsNullOrEmpty(_trace))
        {
            Console.WriteLine("Failed to create trace, trace already existed!");
            return;
        }

        // Format the trace
        _trace = $"#trace _ {stageName} {contextName}.";
    }
